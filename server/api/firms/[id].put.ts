...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (body.name.trim() !== existingFirm.name) {
      const duplicate = await Firm.findOne({ name: body.name.trim() })
      if (duplicate) {
        throw createError({
          statusCode: 409,
    ...
    const updated = await Firm.findByIdAndUpdate(id, updateData, { new: true })
