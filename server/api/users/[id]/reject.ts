...
export default defineEventHandler(async (event) => {
  console.log('Reject user endpoint called');
  ...
  console.log('User ID to reject:', id);
  if (!id) {
    throw createError({
      statusCode: 400,
  ...
  try {
    ...
    // If current user is a manager, they can only reject users in their firm
    if (currentUser.role === 'manager') {
      ...
      if (targetUser.firmId.toString() !== currentUser.firmId.toString()) {
        ...
        throw createError({
          statusCode: 403,
          statusMessage: 'Forbidden: You can only reject users in your firm'
        });
      }

      // Managers can only reject users with 'user' role
      console.log('Checking if target user has user role');
      if (targetUser.role !== 'user') {
        console.log('Target user is not a user role');
        throw createError({
          statusCode: 403,
          statusMessage: 'Forbidden: Managers can only reject users with user role'
        });
      }
    }

    // Update user status to rejected (-1)
    console.log('Updating user status to rejected (-1)');
    targetUser.status = -1;
    ...
    console.log('Error in reject user endpoint:', error);
