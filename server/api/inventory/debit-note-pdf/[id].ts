// server/api/inventory/debit-note-pdf/[id].ts
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const doc = new PDFDocument({
      ...
      info: {
        Title: `Debit Note ${bill.bno}`,
        Author: firm.name,
        Subject: 'Debit Note',
        Keywords: 'debit note, bill, pdf',
    ...
    throw createError({
      statusCode: 500,
      statusMessage: `Error generating debit note PDF: ${error.message || 'Unknown error'}`
    });
  }
});
