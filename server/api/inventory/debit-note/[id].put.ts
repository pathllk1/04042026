// server/api/inventory/debit-note/[id].put.ts
...
import { createDebitNoteLedgerTransactionMongo } from '../../../utils/inventoryLedgerMongo';
  ...
  try {
    ...
    try {
      ...
      if (!existingBill) {
        throw createError({
          statusCode: 404,
          statusMessage: 'Not Found: Debit note not found'
        });
      }
      ...
      if (existingBill.status === 'CANCELLED') {
        throw createError({
          statusCode: 400,
          statusMessage: 'Bad Request: Cannot update a cancelled debit note'
        });
      }
      ...
      for (const stockReg of existingStockRegs) {
        if (stockReg.stockId) {
          // Revert the quantity (add back what was previously subtracted)
      ...
      for (const item of body.stockItems) {
        // For debit note, we're removing items from inventory, so we subtract from stock
        ...
        // For debit note, we subtract the quantity from stock
        const stockUpdate = {
          $inc: { qty: -item.qty },
          $set: stockUpdateSet
        };
      ...
      const ledgerResult = await createDebitNoteLedgerTransactionMongo(updatedBill, user._id.toString(), user.firmId.toString());
