// server/api/inventory/debit-note.ts
...
export default defineEventHandler(async (event) => {
  ...
  try {
    // Get all debit note bills for the user's firm
    const debitNoteBills = await Bills.find({ 
      firm: firmId,
      btype: 'DEBIT NOTE'
    }).sort({ bdate: -1 });

    // Get related stock registrations and parties
    const billIds = debitNoteBills.map(bill => bill._id);
    ...
    // Get unique party IDs from debit note bills
    const partyIds = [...new Set(debitNoteBills.map(bill => bill.partyId))];
    ...
    throw createError({
      statusCode: 500,
      statusMessage: `Error fetching debit note data: ${error.message}`
    });
  }
});