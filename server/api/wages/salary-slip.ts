...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Get request body
    const { wageId, month } = await readBody(event)

    if (!wageId && !month) {
      throw createError({
        statusCode: 400,
        message: 'Either wage ID or month is required'
      })
    }
    ...
    // Find the wage record
    let wage
    if (wageId) {
      // If wageId is provided, get that specific wage
      wage = await Wage.findOne({ _id: wageId, firmId }).lean()
      if (!wage) {
        throw createError({
          statusCode: 404,
      ...
      // If month is provided, get all wages for that month
      ...
      // Return all wages for batch processing
      return {
        success: true,
        wages,
        firm
    ...
    // Get employee details from master roll
    const employee = await MasterRoll.findById(wage.masterRollId).lean()

    // Generate salary slip document
    ...
    event.node.res.setHeader('Content-Disposition', `attachment; filename=salary-slip-${wage.employeeName.replace(/\s+/g, '_')}.docx`)
